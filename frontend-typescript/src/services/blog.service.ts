...
const fetchBlogs = async () =>{
    return await axios.post('http://localhost:8000/api/myblogs',{id:user}).then((res)=>{
        return res.data;
    })
}

const fetchBlogById = async (id:string|number) =>{
    return await axios.post('http://localhost:8000/api/viewblog',{id:user,blog:id}).then((res)=>{ 
        return res.data
    })
}

const livesearch = async (Search:string) =>{
    return await axios.post('http://localhost:8000/api/searchlive',{id:user,name:Search}).then((res)=>{
        return res.data
    })
}

const deleteblog = async (id:string|number) => {
    return await axios.post('http://localhost:8000/api/deleteblog',{id:user,blog:id})
}
